...
{
	W(CSI_VBASE+ch*0x100+CSI_CFG, csi_fmt_cfg->input_fmt << 20 | 
...
void inline bsp_csi_set_buffer_address(unsigned int ch, __csi_buf_t buf, unsigned int addr)
{
    ...
    W(CSI_VBASE+ch*0x100+CSI_BUF0_A + (buf<<2), addr); 
}

unsigned int inline bsp_csi_get_buffer_address(unsigned int ch, __csi_buf_t buf)
{
	unsigned int t;
	t = R(CSI_VBASE+ch*0x100+CSI_BUF0_A + (buf<<2));
	return t;
}
...
void bsp_csi_set_size(unsigned int ch, unsigned int length_h, unsigned int length_v, unsigned int buf_length_y, unsigned int buf_length_c)
{
	/* make sure yuv422 input 2 byte(clock) output 1 pixel */
		...
		t = R(CSI_VBASE+ch*0x100+CSI_RESIZE_H);
		t = (t&0x0000ffff)|(length_h<<16);
    W(CSI_VBASE+ch*0x100+CSI_RESIZE_H, t);
    
    t = R(CSI_VBASE+ch*0x100+CSI_RESIZE_V);
    t = (t&0x0000ffff)|(length_v<<16);
    W(CSI_VBASE+ch*0x100+CSI_RESIZE_V, t);
    
    W(CSI_VBASE+ch*0x100+CSI_BUF_LENGTH, buf_length_y + (buf_length_c<<16));
}
...
{
    W(CSI_VBASE+ch*0x100+CSI_INT_STATUS, interrupt);
}
