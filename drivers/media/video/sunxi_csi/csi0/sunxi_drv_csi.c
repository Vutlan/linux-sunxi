...
static struct csi_fmt formats[] = {
	...
	//mipi 8bit
	{
		.name     		= "planar YUV 422",
		.csi_if				= CSI_IF_MIPI,
		.ccm_fmt			= V4L2_MBUS_FMT_UYVY8_1X1,
		.fourcc   		= V4L2_PIX_FMT_YUV422P,
		.field				= V4L2_FIELD_NONE,
		.input_fmt		= CSI_YUV422,
		.output_fmt		= CSI_FIELD_PLANAR_YUV422,
	...
	},
	{
		.name     		= "RAW Bayer BGGR",
		.csi_if				= CSI_IF_MIPI,
		.ccm_fmt			= V4L2_MBUS_FMT_SBGGR8_1X1,
		.fourcc   		= V4L2_PIX_FMT_SBGGR8,
		.field				= V4L2_FIELD_NONE,
		.input_fmt		= CSI_RAW,
		.output_fmt		= CSI_FIELD_RAW_8,
	...
	//mipi 10bit
	{
		...
		.csi_if				= CSI_IF_MIPI,
		.ccm_fmt			= V4L2_MBUS_FMT_SBGGR10_1X1,
		.fourcc   		= V4L2_PIX_FMT_SBGGR10,
		.field				= V4L2_FIELD_NONE,
		.input_fmt		= CSI_RAW,
		.output_fmt		= CSI_FIELD_RAW_10,
		.mipi_pkt_fmt = RAW10,
		.csi_field		= CSI_ODD,
		.depth    		= 16,
		.planes_cnt		= 1,
	...
	//mipi 12bit
	{
		...
		.csi_if				= CSI_IF_MIPI,
		.ccm_fmt			= V4L2_MBUS_FMT_SBGGR12_1X1,
		.fourcc   		= V4L2_PIX_FMT_SBGGR12,
		.field				= V4L2_FIELD_NONE,
		.input_fmt		= CSI_RAW,
		.output_fmt		= CSI_FIELD_RAW_12,
		.mipi_pkt_fmt = RAW12,
		.csi_field		= CSI_ODD,
		.depth    		= 16,
		.planes_cnt		= 1,
...
static int get_mbus_config(struct csi_dev *dev, struct v4l2_mbus_config *mbus_config)
{
	int ret;
	
	ret = v4l2_subdev_call(dev->sd,video,g_mbus_config,mbus_config);//linux-3.3
	if (ret < 0) {
		csi_err("v4l2 sub device g_mbus_config error!\n");
		return -EFAULT;
	}
...
static int csi_clk_get(struct csi_dev *dev)
{
  ...
  // for csi mclk output
	dev->csi_module_clk=clk_get(NULL,CLK_MOD_CSI0M);//"mod_csi0m"
					...
					struct v4l2_capability *cap)
{
	...
	strcpy(cap->driver, "sunxi_csi0");
	strcpy(cap->card, "sunxi_csi0");
					...
					struct v4l2_format *f)
{
	...
	if(ret == 0) {
		if(mbus_cfg.type == V4L2_MBUS_CSI2) {
			...
			if((mbus_cfg.flags & V4L2_MBUS_CSI2_4_LANE) == V4L2_MBUS_CSI2_4_LANE)
				dev->lane_num = 4;
			else if((mbus_cfg.flags & V4L2_MBUS_CSI2_3_LANE) == V4L2_MBUS_CSI2_3_LANE)
				dev->lane_num = 3;
			else if((mbus_cfg.flags & V4L2_MBUS_CSI2_2_LANE) == V4L2_MBUS_CSI2_2_LANE)
				dev->lane_num = 2;
			else
				dev->lane_num = 1;
			...
			dev->total_ch = 0;
			ch_num = 0;
			if((mbus_cfg.flags & V4L2_MBUS_CSI2_CHANNEL_0) == V4L2_MBUS_CSI2_CHANNEL_0) {
				dev->ch[ch_num].vc = 0;	
				ch_num++;
				dev->total_ch++;
			} 
			if((mbus_cfg.flags & V4L2_MBUS_CSI2_CHANNEL_1) == V4L2_MBUS_CSI2_CHANNEL_1) {
				dev->ch[ch_num].vc = 1;	
				ch_num++;	
				dev->total_ch++;	
			} 
			if((mbus_cfg.flags & V4L2_MBUS_CSI2_CHANNEL_2) == V4L2_MBUS_CSI2_CHANNEL_2) {
				dev->ch[ch_num].vc = 2;	
				ch_num++;	
				dev->total_ch++;
			} 
			if((mbus_cfg.flags & V4L2_MBUS_CSI2_CHANNEL_3) == V4L2_MBUS_CSI2_CHANNEL_3) {
				dev->ch[ch_num].vc = 3;	
				ch_num++;	
				dev->total_ch++;
			}
			
			csi_print("V4L2_MBUS_CSI2,%dlane,%dchannel\n",dev->lane_num,dev->total_ch);
			
			mipi_bps = 112*1000*1000;

			bsp_mipi_csi_set_dphy_timing(&mipi_bps,DPHY_CLK,0);
			bsp_mipi_csi_set_lane(dev->lane_num);
			bsp_mipi_csi_set_total_ch(dev->total_ch);
			
			dev->ch[0].mipi_pkt_fmt = csi_fmt->mipi_pkt_fmt; //assume that only using 1 channel
			
			for(i=0;i<dev->total_ch;i++) {
				bsp_mipi_csi_set_pkt_header(i, dev->ch[i].vc, dev->ch[i].mipi_pkt_fmt);
				if(f->fmt.pix.field == V4L2_FIELD_NONE)
					bsp_mipi_csi_set_src_type(0,PROGRESSIVE);
				else
					bsp_mipi_csi_set_src_type(0,INTERLACED);
			}
			
			bsp_mipi_csi_dphy_enable();
...
static int csi_close(struct file *file)
{
	...
	bsp_csi_capture_video_stop(dev->cur_ch);
	bsp_csi_close();
	bsp_mipi_csi_protocol_disable();
	bsp_mipi_csi_dphy_disable();
	csi_clk_disable(dev);
...
//		}
		...
		if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		  ...
		  dev->ccm_cfg[0]->reset_io=val.gpio;
		}
		...
		if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		  ...
		  dev->ccm_cfg[0]->standby_io=val.gpio;
		}
		...
		if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		  ...
		  dev->ccm_cfg[0]->power_io=val.gpio;
		}
		...
		if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		  ...
		  dev->ccm_cfg[0]->flash_io=val.gpio;
		}
		...
		if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		  ...
		  dev->ccm_cfg[0]->af_power_io=val.gpio;
		}
	} 
...
//		}
		...
		if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		  ...
		  dev->ccm_cfg[1]->reset_io=val.gpio;
		}
		...
		if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		  ...
		  dev->ccm_cfg[1]->standby_io=val.gpio;
		}
		...
		if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		  ...
		  dev->ccm_cfg[1]->power_io=val.gpio;
		}
		...
		if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		  ...
		  dev->ccm_cfg[1]->flash_io=val.gpio;
		}
		...
		if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		  ...
		  dev->ccm_cfg[1]->af_power_io=val.gpio;
		}
	}
...
static int csi_probe(struct platform_device *pdev)
{
  ...
  //script_item_value_type_e  type;
	...
	bsp_mipi_csi_set_base_addr((unsigned int)dev->regs);
...
//#ifndef CSI_VER_FOR_FPGA
  ...
  script_dump_mainkey("csi0_para");
	...
	if (cnt==0) {
	  ...
	  /* request gpio */ //dont need to request after here, just get_item
  	for(i = 0; i < cnt; i++)
  	{
