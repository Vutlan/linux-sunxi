...
MODULE_DESCRIPTION("A low-level driver for SET SIV121D sensors");
MODULE_LICENSE("GPL");

//for internel driver debug
#define DEV_DBG_EN   		0 
#if(DEV_DBG_EN == 1)		
#define csi_dev_dbg(x,arg...) printk(KERN_INFO"[CSI_DEBUG][SIV121D]"x,##arg)
#else
#define csi_dev_dbg(x,arg...) 
#endif
#define csi_dev_err(x,arg...) printk(KERN_INFO"[CSI_ERR][SIV121D]"x,##arg)
#define csi_dev_print(x,arg...) printk(KERN_INFO"[CSI][SIV121D]"x,##arg)
...
static struct regval_list sensor_default_regs[] = 
{
	...
	{{0x10},{0x80}},                                           
	{{0x11},{0x0c}},//10}},                                           
	{{0x12},{0x64}},//60}},  // D65 target    60                                      
	{{0x14},{0x60}},//5d}},  // A target    5                                      
	{{0x34},{0x96}},                                           
	{{0x40},{0x48}},//40}}, // 40                                         
	{{0x5f},{0x01}},                                           
	{{0x41},{0x28}},//20}},                                          
	...
	{{0x42},{0x44}},//43}},                                           
	{{0x43},{0x20}},                                   
	{{0x44},{0x22}},//11}},  // left R gain[7:4], right R gain[3:0] 	 22					                                          
	{{0x45},{0x22}},//11}},  // top R gain[7:4], bottom R gain[3:0] 		22				                                          
	{{0x46},{0x00}},  // left G gain[7:4], right G gain[3:0] 	                                                    
	{{0x47},{0x11}},//00}},  // top G gain[7:4], bottom G gain[3:0] 	   11                                                   
	...
	{{0x4b},{0x48}},//60}},  // X-axis center low[7:0]				48		                                                        
	{{0x4c},{0xe8}},//e0}},  // Y-axis center low[7:0]		 e8			                                                          
	{{0x4d},{0x84}},//88}},  // Shading Center Gain     80                                                                  
	...
	{{0x91},{0x28}},    //down gain                                                                  
	{{0x92},{0x55}},//44}},    //[7:4] upper coring [3:0] down coring                                       
	{{0x9a},{0x40}},                                           
	{{0x9b},{0x40}},                                           
...
static int sensor_write_im(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char value)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct i2c_msg msg;
...
//	regs.reg_num[0] = 0x04;
//	regs.value[0] = 0x01;		//page 0

	regs.reg_num[0] = 0x00; // changed
	regs.value[0] = 0x01;		//page 0
	ret = sensor_write(sd, regs.reg_num, regs.value);
	if (ret < 0) {
