...
extern int ft5x02_read_reg(struct i2c_client * client, u8 regaddr, u8 * regvalue);
	...
	// }
	// else if(tp_type == 1)//SHENGGUANG
	// {
		// #include "ft5x02_config_8.h"
	// }
	// else if(tp_type == 2)//BAISIRUI
	// {
		// #include "ft5x02_config_9.h"
	// }
/*set tx order
...
EXPORT_SYMBOL_GPL(ft5x02_get_ic_param);
...
static int ChangeToHexData(char *line)
{
    ...
    while(p!=NULL){
    	 if(*p==' ') p=p+1;
       FT5X0X_FW[CNT++]=simple_strtol(p,NULL,10);
        p=strsep(&s,",");
    }
   vfree(s);
   CNT -=1;
...
static int  ReadInitParaFromFile(void)
{
    struct file *fp; 
    mm_segment_t fs; 
    loff_t pos=0; 
    ...
    if(SCRIPT_PARSER_OK != script_parser_fetch_ex("ctp_para", "ft5x0x_Init_para_path", (int *)(&ft5x0x_Init_para_path), &type, sizeof(ft5x0x_Init_para_path)/sizeof(int))){
				pr_err("%s: script_parser_fetch err. \n", __func__);
				return -ENODEV;
		}		
    pr_info("%s: ft5x0x_Init_para_path=%s\n",__func__,ft5x0x_Init_para_path);		
	
    FT5X0X_FW=vmalloc(3000); 
    fp = filp_open(ft5x0x_Init_para_path, O_RDWR | O_CREAT, 0644); 
    if (IS_ERR(fp)) { 
        printk("open file %s error\n",ft5x0x_Init_para_path); 
        return -1; 
    } 
    fs = get_fs(); 
    set_fs(KERNEL_DS); 
   ...
   pr_info("number of FT5X0X_FW should be %d \n",57+(*(FT5X0X_FW+55))*2+(*(FT5X0X_FW+56))*5/2);
   
    pr_info("%s.over:sizeof(FT5X0X_FW)=%d\n",__func__,CNT);
...
static int init_Struct_Param_FT5X02(void)
{
	...
	g_ft5x02_tx_offset=*(FT5X0X_FW+54);
	g_ft5x02_tx_num=*(FT5X0X_FW+55);	
	g_ft5x02_rx_num=*(FT5X0X_FW+56);	

	
	for(i=0;i<g_ft5x02_tx_num;i++)
		*(g_ft5x02_tx_order+i)=*(FT5X0X_FW+57+i);
	for(i=0;i<g_ft5x02_tx_num;i++)
		*(g_ft5x02_tx_cap+i)=*(FT5X0X_FW+57+g_ft5x02_tx_num+i);	
	for(i=0;i<g_ft5x02_rx_num;i++)
		*(g_ft5x02_rx_order+i)=*(FT5X0X_FW+57+g_ft5x02_tx_num*2+i);
	for(i=0;i<g_ft5x02_rx_num/2;i++)
		*(g_ft5x02_rx_offset+i)=*(FT5X0X_FW+57+g_ft5x02_tx_num*2+g_ft5x02_rx_num+i);
	for(i=0;i<g_ft5x02_rx_num;i++)
		*(g_ft5x02_rx_cap+i)=*(FT5X0X_FW+57+g_ft5x02_tx_num*2+g_ft5x02_rx_num*3/2+i);
	return 0;
...
int ft5x02_Init_IC_Param(struct i2c_client *client)
{
	...
	if(time==0){
		...
		if(SCRIPT_PARSER_OK != script_parser_fetch("ctp_para", "ft5x0x_init_if", &ft5x0x_init_if, 1))
			...
			pr_info("%s: ft5x0x_init_if = %d. \n", __func__, ft5x0x_init_if);			
	
		time=1;
	}	

	if(1==ft5x0x_init_if){
		if(0!=ReadInitParaFromFile()){		
			 pr_info("%s:ReadDataFromFile failed\n",__func__); 		
			return -1; 	
		}
		init_Struct_Param_FT5X02();
		vfree(FT5X0X_FW);
		ft5x0x_init_if=0;
	}		
...
EXPORT_SYMBOL_GPL(ft5x02_Init_IC_Param);
