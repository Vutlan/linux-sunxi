...
static int ctp_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	if(twi_id == adapter->nr){
	        ...
	        ret = i2c_read_bytes(client,buf,3);
...
/*
	...
	for (i=0; i<5; i++)
	{
		...
		if(ret < 0)
		{
			dev_info(&ts->client->dev, "GT811 Send config failed %d times!\n", i);
		}else {
			break;
		}
	}
...
*/	
	...
	for(retry=0; retry <5; retry++)
	{
		...
		if (ret > 0) {
			...
			dev_info(&client->dev, "goodix I2C TEST FAILED!Please check the HARDWARE connect\n");
		}
	}
	if(ret <= 0)
	{
