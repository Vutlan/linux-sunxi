...
static const unsigned short i2c_address[] = {0x6a, I2C_CLIENT_END};
...
static int gyr_fetch_sysconfig_para(void)
{
	...
	if(SCRIPT_PARSER_OK != (ret = script_parser_fetch("gyr_para", "gyr_used", &device_used, 1))){
			pr_err("%s: script_parser_fetch err.ret = %d. \n", __func__, ret);
			goto script_parser_fetch_err;
	}
	if(1 == device_used){
		if(SCRIPT_PARSER_OK != script_parser_fetch("gyr_para", "gyr_twi_id", &twi_id, 1)){
