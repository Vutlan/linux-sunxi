	...
	for (i = 0; i < 100; i++) {
		...
		if ((status & RESET_MODE) == 0) {
			...
			if (priv->can.ctrlmode & CAN_CTRLMODE_BERR_REPORTING) {
				writel(0xFF & ~OR_IRQ_EN, CAN_INTEN_ADDR);
			} else {
				writel(0xFF & ~BERR_IRQ_EN & ~OR_IRQ_EN, CAN_INTEN_ADDR);
			}
...
*/
static netdev_tx_t sun7i_can_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	struct sun7i_can_priv *priv = netdev_priv(dev);
...
#if VTDEBUG
	// debug
	cf->data[6] = readl(CAN_RMCNT_ADDR);
	cf->data[7] = readl(CAN_RBUFSA_ADDR);
#endif
	/* release receive buffer */
	sun7i_can_write_cmdreg(priv, RELEASE_RBUF);
	//writel(RELEASE_RBUF, CAN_CMD_ADDR);
...
static int sun7i_can_err(struct net_device *dev, uint8_t isrc, uint8_t status)
{
	...
	if (isrc & DATA_ORUNI) {
		...
		//pr_info("data overrun interrupt isrc=%x rxcnt=%x, rxadr=%x\n", isrc,
		//		readl(CAN_RMCNT_ADDR), readl(CAN_RBUFSA_ADDR));
		...
		//writel(CLEAR_DOVERRUN, CAN_CMD_ADDR);  // clear DATA_OR & RX_RDY
	}
	...
	&& (state == CAN_STATE_ERROR_WARNING || state == CAN_STATE_ERROR_PASSIVE)) {
...
irqreturn_t sun7i_can_interrupt(int irq, void *dev_id)
{
	...
	while ((isrc = readl(CAN_INT_ADDR)) && (n < SUN7I_CAN_MAX_IRQ)) {
		...
		//pr_info("sun7ican: capture a interrupt: isrc=%x, isrc_reg=%x, en=%x, st=%x, st_reg=%x rxcnt=%x, rxadr=%x\n",
		//			isrc, readl(CAN_INT_ADDR), readl(CAN_INTEN_ADDR),
		//			status, readl(CAN_STA_ADDR),
		//			readl(CAN_RMCNT_ADDR), readl(CAN_RBUFSA_ADDR));
		...
		if (isrc & RBUF_VLD) {
			...
			while (status & RBUF_RDY) {        //RX buffer is not empty
				...
				sun7i_can_rx(dev);
#else
				pr_info("data read dummy isrc=%x en=%x, rxcnt=%x, rxadr=%x\n",
									readl(CAN_INT_ADDR), readl(CAN_INTEN_ADDR),
									readl(CAN_RMCNT_ADDR), readl(CAN_RBUFSA_ADDR));
				writel(RELEASE_RBUF, CAN_CMD_ADDR);
				writel(RBUF_VLD, CAN_INT_ADDR);
#endif
				status = readl(CAN_STA_ADDR);
