...
int gsensor_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	if(twi_id == adapter->nr){
	    ...
	    printk("ret :%d",ret);
	    if (((ret &0x00FF) == KIONIX_ACCEL_WHO_AM_I_KXTIK) || ((ret &0x00FF) == KIONIX_ACCEL_WHO_AM_I_KXTJ9)){
        		 pr_info("%s:this accelerometer is a KXTIK.\n",__func__);
            	 strlcpy(info->type, SENSOR_NAME, I2C_NAME_SIZE);
                 return 0;

        }else{
            pr_info("%s: kionix Sensor Device not found, \
...
static DEVICE_ATTR(enable, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		NULL, kxtik_enable_store);

static DEVICE_ATTR(delay, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		NULL,  kxtik_delay_store);
				   ...
				   const struct i2c_device_id *id)
{
	...
	printk("<%s> kxtik_init_client result %d\n", __func__, result);
	if(result != 0)
	{
...
static int __init kxtik_init(void)
{
	...
	printk(KERN_INFO "add kxtik i2c driver\n");
