	...
	if(twi_id == adapter->nr){
	    ...
	    {	    client->addr = i2c_address[i2c_num];
    	       ...
    	       if ((ret &0x00FF) == MMA8652_ID){
            		pr_info("Freescale Sensortec Device detected!\n" );
				   ...
				   const struct i2c_device_id *id)
{
	...
	printk( "mma865x probe i2c address is %d \n",i2c_address[i2c_num]);
	client->addr =i2c_address[i2c_num];
...
{
	printk("remove mma865x i2c driver.\n");
