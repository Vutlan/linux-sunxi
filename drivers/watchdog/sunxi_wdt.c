...
#define WDT_INDEX 	1 		/* watchdog index, on sun6i platform, 1 ~ 4 */
...
enum sunxi_watchdog_func {
	TO_CPU = 0,		/* reset the single cpu core */
	TO_WHOLE_SYSTEM,	/* reset the whole system */
	ONLY_INTERRUPT,		/* only generate interrupt, not reset */
};
...
#define WDT_REG_OFFSET (0xA0 + ((WDT_INDEX - 1) << 5)) /* 0xA0 + (index - 1) * 0x20 */
#else
#error "err: no valid platform defined, eg: sun4i/sun5i/sun6i/sun7i!"
#endif
...
static struct resource sunxi_wdt_res[] = {
	{
		...
		.end	= AW_TIMER_BASE + WDT_REG_OFFSET + 0x20, /* liugang, for sun6i, 2012-9-17 */
...
static bool wdt_get_irq_pend(void)
{
	bool 	bret = false;
 ...
 *
 * Returns <0 if failed, otherwise return the actually timeout value set.
 */
...
static int sunxi_wdt_release(struct inode *inode, struct file *file)
{
	if(expect_release)
		...
		watchdog_kick(); /* why here? liugang 2012-9-17 */
	}
...
static int __devinit sunxi_wdt_probe(struct platform_device *pdev)
{
	...
	watchdog_probe_init(); /* liugang, 2012-9-17 */
	return ret;
