...
void em55_power(struct sw_modem *modem, u32 on)
{
    ...
    if(on){
    	/* power on bat */
        modem_reset(modem,1);//let the BB-RST pin high
        modem_sleep(modem, 0);//wake up baseband by pull down the INT1 pin high
		modem_vbat(modem, 1);//power the modem by open the battery path
		
		msleep(100);
        modem_power_on_off(modem, 0);//PWR_EN
        sw_module_delay(1800);
        modem_power_on_off(modem, 1);//PWR_EN
