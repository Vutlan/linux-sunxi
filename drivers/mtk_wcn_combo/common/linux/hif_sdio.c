    ...
    )
{
    ...
    HIF_SDIO_INFO_FUNC("vendor(0x%x) device(0x%x) num(0x%x) quirks(0x%08x)\n",
        func->vendor, func->device, func->num, func->card->quirks);
    ...
    )
{
    ...
    do {
        ...
        sdio_release_host(func);
        HIF_SDIO_INFO_FUNC("sdio_disable_func(0x%p) ret(%d)\n", func, ret);
    }while(0);
