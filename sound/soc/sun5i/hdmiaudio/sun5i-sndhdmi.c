/*
 * sound\soc\sun5i\hdmiaudio\sun5i-sndhdmi.c
...
static int sun5i_sndhdmi_startup(struct snd_pcm_substream *substream)
{
	int ret = 0;
	#ifdef ENFORCE_RATES
...
static void sun5i_sndhdmi_shutdown(struct snd_pcm_substream *substream)
{
	mutex_lock(&clk_lock);
	clk_users -= 1;
	if (clk_users == 0) {
...
static int sun5i_sndhdmi_hw_params(struct snd_pcm_substream *substream,
...
static struct snd_soc_ops sun5i_sndhdmi_ops = {
	.startup 	= sun5i_sndhdmi_startup,
	.shutdown 	= sun5i_sndhdmi_shutdown,
	.hw_params 	= sun5i_sndhdmi_hw_params,
};

static struct snd_soc_dai_link sun5i_sndhdmi_dai_link = {
	.name 			= "HDMIAUDIO",
	.stream_name 	= "SUN5I-HDMIAUDIO",
	.cpu_dai_name 	= "sun5i-hdmiaudio.0",
	.codec_dai_name = "sndhdmi",
	.platform_name 	= "sun5i-hdmiaudio-pcm-audio.0",
	.codec_name 	= "sun5i-hdmiaudio-codec.0",
	.ops 			= &sun5i_sndhdmi_ops,
};
...
static struct platform_device *sun5i_sndhdmi_device;

static int __init sun5i_sndhdmi_init(void)
{
	int ret;

	sun5i_sndhdmi_device = platform_device_alloc("soc-audio", 0);
		
	if(!sun5i_sndhdmi_device)
		return -ENOMEM;
			
	platform_set_drvdata(sun5i_sndhdmi_device, &snd_soc_sun5i_sndhdmi);
		
	ret = platform_device_add(sun5i_sndhdmi_device);		
		
	if (ret) {			
		platform_device_put(sun5i_sndhdmi_device);
	}

	return ret;
}

static void __exit sun5i_sndhdmi_exit(void)
{
	platform_device_unregister(sun5i_sndhdmi_device);
}

module_init(sun5i_sndhdmi_init);
module_exit(sun5i_sndhdmi_exit);

MODULE_AUTHOR("ALL WINNER");
MODULE_DESCRIPTION("SUN5I_SNDHDMI ALSA SoC audio driver");
MODULE_LICENSE("GPL");
