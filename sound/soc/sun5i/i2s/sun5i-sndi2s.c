...
static int sun5i_sndi2s_startup(struct snd_pcm_substream *substream)
{
	int ret = 0;
	#ifdef ENFORCE_RATES
...
static void sun5i_sndi2s_shutdown(struct snd_pcm_substream *substream)
{
	mutex_lock(&clk_lock);
	clk_users -= 1;
	if (clk_users == 0) {
...
static struct snd_soc_ops sun5i_sndi2s_ops = {
	.startup 		= sun5i_sndi2s_startup,
	.shutdown 		= sun5i_sndi2s_shutdown,
	.hw_params 		= sun5i_sndi2s_hw_params,
};

static struct snd_soc_dai_link sun5i_sndi2s_dai_link = {
	.name 			= "I2S",
	.stream_name 	= "SUN5I-I2S",
	.cpu_dai_name 	= "sun5i-i2s.0",
	.codec_dai_name = "sndi2s",
	.platform_name 	= "sun5i-i2s-pcm-audio.0",	
	.codec_name 	= "sun5i-i2s-codec.0",
	.ops 			= &sun5i_sndi2s_ops,
};
...
static struct platform_device *sun5i_sndi2s_device;
...
static void __exit sun5i_sndi2s_exit(void)
{
	if(i2s_used) {
		i2s_used = 0;
		platform_device_unregister(sun5i_sndi2s_device);
	}
}

module_init(sun5i_sndi2s_init);
module_exit(sun5i_sndi2s_exit);

MODULE_AUTHOR("ALL WINNER");
MODULE_DESCRIPTION("SUN5I_sndi2s ALSA SoC audio driver");
MODULE_LICENSE("GPL");
