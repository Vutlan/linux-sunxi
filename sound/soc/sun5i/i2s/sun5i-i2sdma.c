...
static const struct snd_pcm_hardware sun5i_pcm_play_hardware = {
...
static const struct snd_pcm_hardware sun5i_pcm_capture_hardware = {
	.info			= SNDRV_PCM_INFO_INTERLEAVED | SNDRV_PCM_INFO_BLOCK_TRANSFER |
				      SNDRV_PCM_INFO_MMAP | SNDRV_PCM_INFO_MMAP_VALID |
				      SNDRV_PCM_INFO_PAUSE | SNDRV_PCM_INFO_RESUME,
	.formats		= SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S20_3LE | SNDRV_PCM_FMTBIT_S24_LE,
	...
	struct snd_pcm_hw_params *params)
{
    ...
    struct sun5i_dma_params *play_dma = NULL;
    struct sun5i_dma_params *capture_dma = NULL;
...
static int sun5i_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		snd_soc_set_runtime_hwparams(substream, &sun5i_pcm_play_hardware);
		...
		snd_soc_set_runtime_hwparams(substream, &sun5i_pcm_capture_hardware);
...
static int sun5i_pcm_preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		size = sun5i_pcm_play_hardware.buffer_bytes_max;
	} else {
		size = sun5i_pcm_capture_hardware.buffer_bytes_max;
	}
