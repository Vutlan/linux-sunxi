...
static int codec_voice_record_enable = 0;
...
static int codec_headphone_play_open(void)
{
	...
	if (codec_lineinin_enabled){
		printk("codec_lineinin_enabled = true\n");
		return 0;
	}
	type = script_get_item("audio_para", "headphone_vol", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (codec_fm_speaker_enabled) {
		type = script_get_item("audio_para", "fm_speaker_vol", &val);
		if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
			printk("[audiocodec] pa_single_vol type err!\n");
			val.val = 0x15;
		}
		pa_vol = val.val;
	        printk("codec_set_lineinin speaker vol: %d\n", val.val);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (codec_fm_headset_enabled) {
		type = script_get_item("audio_para", "fm_headset_vol", &val);
		if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
			printk("[audiocodec] fm_headset_vol type err!\n");
			val.val = 0x30;
		}
		headphone_vol = val.val;

	        printk("codec_set_lineinin headset vol: %d\n", val.val);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	type = script_get_item("audio_para", "phone_speaker_vol", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
	        printk("[audiocodec] phone_speaker_vol type err!\n");
		val.val = 0x1b;
	}
	...
	if (codec_speakerout_enabled) {
		...
		codec_wr_control(SUN6I_MIC_CTRL, 0x1f, LINEOUT_VOL, pa_vol);
		//lkjcodec_wr_control(SUN6I_DAC_ACTL, 0x1, LHPIS, 0x1);
		//lkjcodec_wr_control(SUN6I_DAC_ACTL, 0x1, RHPIS, 0x1);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (codec_headphoneout_enabled) {
		type = script_get_item("audio_para", "phone_headset_vol", &val);
		if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
			printk("[audiocodec] phone_headset_vol type err!\n");
			val.val = 0x3a;
		}
		headphone_vol = val.val;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (codec_earpieceout_enabled) {
		type = script_get_item("audio_para", "phone_earpiece_vol", &val);
		if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
			printk("[audiocodec] phone_earpiece_vol type err!\n");
			val.val = 0x3a;
		}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (codec_adcphonein_enabled) {
	   //ret = codec_adcphonein_open();
		codec_wr_control(SUN6I_ADC_ACTL, 0x1, LADCMIXMUTEPHONEPN, 0x1);
	 	if (codec_voice_record_enable){
			printk("fun:%s, line:%d -----record\n", __func__, __LINE__);
			codec_wr_control(SUN6I_ADC_ACTL, 0x7f, 7, 0x12);//r phonepn 001 0010, r  mixer
		}
	} else {
		/*disable PHONEP-PHONEN Boost stage*/
		codec_wr_control(SUN6I_ADC_ACTL, 0x1, RADCMIXMUTEPHONEPN, 0x0);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	printk("[audiocodec] codec_set_phonemic type val.val=%d\n", val.val);
...
//		codec_wr_control(SUN6I_MIC_CTRL, 0x7,MIC1BOOST, 0);
		...
		codec_wr_control(SUN6I_MIC_CTRL, 0x1, PHONEOUTS0, 0x1);
	 	...
	 	if (codec_voice_record_enable){
			printk("fun:%s, line:%d -----record\n", __func__, __LINE__);
			codec_wr_control(SUN6I_ADC_ACTL, 0x7f, 7, 0x50);
		}
	} else {
		/*disable mic pa*/
		codec_wr_control(SUN6I_MIC_CTRL, 0x1, MIC1AMPEN, 0x0);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (codec_voice_record_enable){
	 	if (codec_phonemic_enabled) {
			printk("2 main mic record\n");
			codec_wr_control(SUN6I_ADC_ACTL, 0x7f, 7, 0x50);//RADCMIXMUTE r phonepn 101 0000, r main mic1	
		} else if (codec_headsetmic_enabled){
			printk("headsetmic  record\n");
			codec_wr_control(SUN6I_ADC_ACTL, 0x7f, 7, 0x30);//r phonepn 011 0000, r  mic2	
		} else if (codec_dacphoneout_enabled){
			printk("bluetooth  record\n");
			codec_wr_control(SUN6I_ADC_ACTL, 0x7f, 7, 0x12);//r phonepn 001 0010, r  mixer
		}
       } else { 
			printk("disable  record\n");
		codec_wr_control(SUN6I_ADC_ACTL, 0x7f, 7, 0x10);// r phonepn 001 0000
	}
	return 0;
}
...
/*
*	codec_headsetmic_enabled == 1, open mic1.
*	codec_headsetmic_enabled == 0, close mic1.
*/
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (codec_headsetmic_enabled) {
		...
		codec_wr_control(SUN6I_MIC_CTRL, 0x7,MIC2BOOST,mic_val);
		/*select mic2 source */
		codec_wr_control(SUN6I_MIC_CTRL, 0x1, MIC2_SEL, 0x1);
		//codec_wr_control(SUN6I_PA_CTRL, 0x7, MIG2G, 0x1);
		...
		/*select mic2 source */
		//lkjcodec_wr_control(SUN6I_MIC_CTRL, 0x1, MIC2_SEL, 0x0);
	}
...
static const struct snd_kcontrol_new codec_snd_controls[] = {
	...
	*/
	...
	SOC_SINGLE_BOOL_EXT("Audio phone mic", 0, codec_get_phonemic, codec_set_phonemic), 		/*set mic*/
	SOC_SINGLE_BOOL_EXT("Audio phone headsetmic", 0, codec_get_headsetmic, codec_set_headsetmic),    	/*set headset mic*/
...
static int snd_sun6i_codec_prepare(struct snd_pcm_substream	*substream)
{
   ...
   if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK){
		...
		if (codec_voice_record_enable || codec_adcphonein_enabled) {
			printk("codec_voice_record_and_adcphonein_open capture\n");
	   		codec_voice_record_and_adcphonein_open();
		} else {
			printk("normal record capture\n");
	   		codec_capture_open();
		}
