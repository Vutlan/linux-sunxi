...
static script_sub_key_old_t *sw_cfg_get_subkey(const char *script_buf, const char *main_key, const char *sub_key)
{
    script_head_t *hd = (script_head_t *)script_buf;
    script_main_key_old_t *mk = (script_main_key_old_t *)(hd + 1);
    script_sub_key_old_t *sk = NULL;
    int i, j;
    ...
    for (i = 0; i < hd->main_key_count; i++) {
        ...
        for (j = 0; j < mk->lenth; j++) {
            sk = (script_sub_key_old_t *)(script_buf + (mk->offset<<2) + j * sizeof(script_sub_key_old_t));
...
int script_parser_fetch(char *main_name, char *sub_name, int value[], int count)
{
    ...
    script_main_key_old_t  *main_key = NULL;
    script_sub_key_old_t   *sub_key = NULL;
    int    i, j;
    ...
    {
        main_key = (script_main_key_old_t *)(script_mod_buf + (sizeof(script_head_t)) + i * sizeof(script_main_key_old_t));
        ...
        {
            sub_key = (script_sub_key_old_t *)(script_mod_buf + (main_key->offset<<2) + (j * sizeof(script_sub_key_old_t)));
