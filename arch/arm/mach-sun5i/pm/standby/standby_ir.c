...
#if 0
/*使用cpu搬运数据*/
__s32 standby_ir_get_data(void)
{
    unsigned long k = 0;
    ...
    IR_reg = (__standby_ir_reg_t *)SW_VA_IR0_IO_BASE;
    ir_buffer.dcnt = 0;
    nbytes = (IR_reg->CIR_RxSta & 0x7f00)>>8;

    while((IR_reg->CIR_RxSta & 3) == 0 && (ir_buffer.dcnt < 128))
    {
        if ((IR_reg->CIR_RxSta & 0x10)!=0 && (IR_reg->CIR_RxSta & 0x7f00)!=0)
        {
            ir_buffer.buf[k++] = (unsigned char)(IR_reg->CIR_RxFifo);
            ir_buffer.dcnt ++;
            if (--nbytes == 0)
            {
                nbytes = (IR_reg->CIR_RxSta & 0x7f00)>>8;
